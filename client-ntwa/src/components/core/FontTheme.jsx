        ...
        <section className="w-375 bg-white rounded-t-lg pt-3 px-4 border border-black md:w-768 lg:w-950 min-h-620 ">
            ...
            <form>
                <label className='flex flex-row justify-between items-center h-72 w-full border border-black rounded-md px-3 my-3'>
                    <img src={sansserif_icon} alt='font icon' className='h-10 w-10 rounded-xl border border-black'/>
                    <div className='flex flex-col w-56'>
                        <span className='font-bold text-sm'>Sans-Serif</span>
                        <span className='text-xs'>Clean and modern, easy to read</span>
                    </div>
                    
                    <input type="radio" name='font' value='sans' checked={font === 'sans'} onChange={(e) => setFont(e.target.value)}/>
                </label>

                <label className='flex flex-row justify-between items-center h-72 w-full border border-black rounded-md px-3 mb-3'>
                    <img src={serif_icon} alt='' className='h-10 w-10 rounded-xl border border-black'/>
                    <div className='flex flex-col w-56'>
                        <span className='font-bold text-sm'>Serif</span>
                        <span className='text-xs  '>Classic and elegant for a timeless fell</span>
                    </div>
                    
                    <input type="radio" name='font' value='serif' checked={font === 'serif'} onChange={(e) => setFont(e.target.value)} />
                </label>

                <label className='flex flex-row justify-between items-center h-72 w-full border border-black rounded-md px-3 mb-3'>
                    <img src={monospace_icon} alt='' className='h-10 w-10 rounded-xl border border-black'/>
                    <div className='flex flex-col w-56'>
                        <span className='font-bold text-sm'>Monospace</span>
                        <span className='text-xs'>Code-like, great for technical vibe</span>
                    </div>
                    
                    <input type="radio" name='font' value='mono' checked={font === 'mono'} onChange={(e) => setFont(e.target.value)}/>
                </label>
            </form>
        </section>
    )
